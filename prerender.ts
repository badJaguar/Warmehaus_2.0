          ...
          if (ts.isCallExpression(decoratorNode) &&
            ...
            decoratorNode.arguments.forEach(node => {
              if (ts.isObjectLiteralExpression(node)) {
                ...
                const identifierNodes = ((<ts.PropertyAssignment>importsNode).initializer as ts.ArrayLiteralExpression).elements.filter(node => {
                ...
                identifierNodes.forEach(node => {
                  if (ts.isCallExpression(node)) {
                    ...
                    node = (node.expression as ts.PropertyAccessExpression).expression;
                  }
                  identifiers.push((node as ts.Identifier).escapedText);
                });
              }
            });
          }
